		...
		try
		{
			for( String fieldName : NodeClassUtil.getAllNodeFieldNames( node.getClass() ) )
				if( AnnotationsParser.getAttributes( node.getClass(), fieldName ).getEncrypted() )
				{
					...
					if( encrypted.startsWith( EncryptionUtil.MD5_PREFIX ) )
						return;
...
//		Environment.addProperty( VolatileFile.STORAGE_LOCATION_PROPERTY, "/var/lib/fantasystep/treemanager" );
		...
		{
			logger.info( String.format( "Setting up StorageHandler: %s", handler.getClass().getCanonicalName() ) );
			handler.setup();
		}
...
//		proxy.init();
//		try {
//			proxy.authenticate("demo", "demo");
//		} catch (InvalidCredentialsException e) {
//			e.printStackTrace();
//		}
//		Node a = proxy.getNodeByID(UUID.fromString("35abeee1-1a7d-457a-949f-e956feff3c51"));
//		Node node = proxy.getFullNodeByID(UUID.fromString("72bedae8-85e0-11e4-928e-0242ac11000b"));
//		((User)node).setPassword("Binbin526");
//		User user = new User();
